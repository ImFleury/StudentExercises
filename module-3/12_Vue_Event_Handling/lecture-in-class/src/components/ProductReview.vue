<template>
  <div class="main">
    ...
    <div class="well-display">
      ...
      <div v-for="item in 5" :key="item" class="well">
          <span class="amount" v-on:click="filter = item">{{ numberOfReviews(item) }}</span>
           {{item}} Star Review{{  numberOfReviews(item)  === 1 ? "" : "s" }}
      </div>
...
export default {
  ...
  computed: {
    ...
    averageRating() {
      let sum = this.reviews.reduce((currentSum, review) => {
        return currentSum + parseInt(review.rating);
      }, 0);
  ...
  methods: {
    numberOfReviews(n){
       return this.reviews.reduce((currentCount, review) => {
        return currentCount + (review.rating === n);
      }, 0);
    },
    addNewReview(){
