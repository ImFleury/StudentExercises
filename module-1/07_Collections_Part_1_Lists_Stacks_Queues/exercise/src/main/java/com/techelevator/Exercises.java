	...
	public List<String> no4LetterWords(String[] stringArray) {
		...
		for (int i = 0; i < stringArray.length; i++) {
			if (stringArray[i].length() != 4) {
				returningList.add(stringArray[i]);
			}
		}
...
*/
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) {
		...
		for (Integer numberDivided: intArray) {
			doublesDivided.add(numberDivided / 2.0 );
		}
	 ...
	 */
	public List<Integer> oddOnly(Integer[] integerArray) {List<Integer> onlyOdds = new ArrayList<Integer>();

		for (Integer theOdds: integerArray){

			if (theOdds % 2 != 0) {
				onlyOdds.add(theOdds);
			}
	...
	** INTERVIEW QUESTION **
	 ...
	 */
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {		List<Integer> interLeave = new ArrayList<Integer>();
