...
public class Exercises {
	...
	public String animalGroupName(String animalName) {
		...
		return animalMap.get(animalName.toUpperCase());
	}
	...
	public Map<String, Integer> last2Revisited(String[] words) {
		...
		// determine count for each string in the array
		for (String word : words) {

			// check if word is not already a key in the hashmap
			if (!keyAndCount.containsKey(word)) {

				// check if length of word is 2 or less - automatically 0
				int wordLength = word.length();
				if (wordLength <= 2) {
					keyAndCount.put(word, 0);
				} else { // word length greater than 2
					String lastTwo = word.substring(wordLength - 2);
					int lastTwoCount = 0;
					// for each substring of 2 (not including last one), check if equal to last 2 char
					for (int i = 0; i < wordLength - 2; i++) {
						String substringTwo = word.substring(i, i + 2);
						if (substringTwo.equals(lastTwo)) {
							lastTwoCount++;
						}
					}
					// add word and count to hashmap
					//System.out.println(word + " " + lastTwoCount);
					keyAndCount.put(word, lastTwoCount);
