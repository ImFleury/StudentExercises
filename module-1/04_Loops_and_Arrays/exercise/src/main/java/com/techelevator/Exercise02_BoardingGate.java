...
public class Exercise02_BoardingGate {
    ...
    public int getNumberOfFullRows(boolean[] seatingChart) {
        ...
        // Check each seat in the seating chart
        for (int i = 0; i < seatingChart.length; i++){

            // setting currentSeatCount to 0 if it's the start of a new row
            if (i % 3 == 0){
                currentSeatCount = 0;
            }

            // if the seat at position i is taken, then increase the row's currentSeatCount by 1
            if (seatingChart[i] == false){
                currentSeatCount++;
            }
            // if all seats in the current row are false(taken) the fullRows counter goes up by 1
            if (currentSeatCount == 3){
                fullRows++;
            }
