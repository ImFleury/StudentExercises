...
public int getCheesePizzaRevenue(int[] orders) {
        ...
        int smallCheeseRevenue = 8 * smallCheeseCount;
        int mediumCheeseRevenue = 11 * mediumCheeseCount;
        int largeCheeseRevenue = 14 * largeCheeseCount;
        int totalCheeseRevenue = smallCheeseRevenue + mediumCheeseRevenue + largeCheeseRevenue;
        return totalCheeseRevenue;
    }
}*/ // Method 2 I used to solve.
    public int getCheesePizzaRevenue(int[] orders) {
        int cheeseRevenue = 0;
        for (int i = 0; i < orders.length; i++) {
            if (orders[i] == SMALL_CHEESE) {
                cheeseRevenue = cheeseRevenue + 8;
            }
            if (orders[i] == MEDIUM_CHEESE) {
                cheeseRevenue = cheeseRevenue + 11;
            }
            if (orders[i] == LARGE_CHEESE) {
                cheeseRevenue = cheeseRevenue + 14;
            }
        }
