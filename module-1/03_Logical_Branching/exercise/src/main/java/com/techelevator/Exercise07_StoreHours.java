...
public class Exercise07_StoreHours {
    ...
    public boolean isStoreOpen(int currentHour, char currentDay) {
        ...
        char wed = 'W';
        char fri = 'F';
        if (currentHour >= 8 && currentHour < 17 && (currentDay == mon || currentDay == wed || currentDay == fri)) {
            return true;
        }
        return false;
    }
    ...
    public boolean isStoreOpen(int currentHour, char currentDay, boolean isSummer) {
        ...
        if (isSummer) {
            if ((currentHour >= 8 && currentHour < 20 && currentDay == wed) || (currentHour >= 9 && currentHour < 15 && currentDay == sat)) {
                return true;
            }
        }
        return false;
