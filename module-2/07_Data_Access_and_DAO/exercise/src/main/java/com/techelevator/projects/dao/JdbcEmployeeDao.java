...
public class JdbcEmployeeDao implements EmployeeDao {
	...
	public void addEmployeeToProject(int projectId, int employeeId) {
		...
		Integer newProjectId = jdbcTemplate.queryForObject(sql, Integer.class, projectId, employeeId);
	}
	...
	public List<Employee> getEmployeesWithoutProjects() {
		...
		List<Employee> employees = new ArrayList<>();
		SqlRowSet results = jdbcTemplate.queryForRowSet("SELECT * FROM employee WHERE employee_id NOT IN (SELECT employee_id FROM project_employee);");
		while (results.next()) {
			employees.add(mapRowToEmployee(results));
		}
		return employees;
